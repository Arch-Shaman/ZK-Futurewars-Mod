...
function gadgetHandler:RegisterCMDID(gadget, id)
	if not id then
		Spring.Log(HANDLER_BASENAME, LOG.ERROR, 'Gadget (' .. gadget.ghInfo.name .. ') ' .. 'tried to register a NIL CMD_ID')
	else
		if (id < 1000) then
			Spring.Log(HANDLER_BASENAME, LOG.ERROR, 'Gadget (' .. gadget.ghInfo.name .. ') ' .. 'tried to register a reserved CMD_ID')
			Script.Kill('Reserved CMD_ID code: ' .. id)
		end

		if (self.CMDIDs[id] ~= nil) then
			Spring.Log(HANDLER_BASENAME, LOG.ERROR, 'Gadget (' .. gadget.ghInfo.name .. ') ' .. 'tried to register a duplicated CMD_ID')
...
function gadgetHandler:AllowUnitTransportLoad(transporterID, transporterUnitDefID, transporterTeam, transporteeID, transporteeUnitDefID, transporteeTeam, loadPosX, loadPosY, loadPosZ)
	for _,g in r_ipairs(self.AllowUnitTransportLoadList) do
		if (not g:AllowUnitTransportLoad(transporterID, transporterUnitDefID, transporterTeam, transporteeID, transporteeUnitDefID, transporteeTeam, loadPosX, loadPosY, loadPosZ)) then
			return false
		end
	end
	return true
...
function gadgetHandler:UnitPreDamaged(unitID, unitDefID, unitTeam, damage, paralyzer, weaponDefID, projectileID, attackerID, attackerDefID, attackerTeam)
	...
	if gadgets then
		...
		for i = 1, gadgets.count do
			g = data[i]
			local dam, imp = g:UnitPreDamaged(unitID, unitDefID, unitTeam, rDam, paralyzer, weaponDefID, attackerID, attackerDefID, attackerTeam, projectileID)
			if (dam ~= nil) then
				rDam = dam
			end
			if (imp ~= nil) then
...
function gadgetHandler:UnitDamaged(unitID, unitDefID, unitTeam, damage, paralyzer, weaponID, projectileID,attackerID, attackerDefID, attackerTeam)
	...
	for i = 1, UnitDamaged_count do
		...
		g:UnitDamaged(unitID, unitDefID, unitTeam, damage, paralyzer, weaponID, attackerID, attackerDefID, attackerTeam)
	end
	return
end
...
function gadgetHandler:FeaturePreDamaged(featureID, featureDefID, featureTeam, damage, weaponDefID, projectileID, attackerID, attackerDefID, attackerTeam)
	...
	if gadgets then
		...
		for i = 1, gadgets.count do
			g = data[i]
			local dam, imp = g:FeaturePreDamaged(featureID, featureDefID, featureTeam, damage, weaponDefID, projectileID, attackerID, attackerDefID, attackerTeam)
			if (dam ~= nil) then
				rDam = dam
			end
			if (imp ~= nil) then
