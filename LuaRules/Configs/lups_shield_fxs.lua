...
for unitDefID = 1, #UnitDefs do
	...
	if params.shield_radius then
		local radius = tonumber(params.shield_radius)
		...
		myShield.rechargeDelay = tonumber(params.shield_recharge_delay) or 0

		myShield.size = radius
		myShield.radius = radius
		myShield.pos = {0, tonumber(params.shield_emit_height) or 0, tonumber(params.shield_emit_offset) or 0}

		local strengthMult = tonumber(params.shield_color_mult)
		...
		local shieldPower = tonumber(params.shield_power)
		...
		if (GG.Lups.Config.quality or 2) >= 3 then
			...
			if isChicken then
				local cmap = myShield.colormap1
				cmap[1][4] = 1.5 -- Note that alpha is multiplied by 0.26
				cmap[2][4] = 1.5
				myShield.texture = "bitmaps/GPL/bubbleShield.png"
		...
		shieldUnitDefs[unitDefID] = {
			...
			shieldCapacity = tonumber(params.shield_power),
			damageMultShieldCapacity = tonumber(params.shield_power_gfx_override or params.shield_power),
