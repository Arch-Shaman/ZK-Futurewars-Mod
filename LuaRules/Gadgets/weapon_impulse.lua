...
for i = 1, #WeaponDefs do
	...
	if wd.customParams and wd.customParams.impulse then
		...
		if wd.customParams.impulsemaxdepth and wd.customParams.impulsedepthmult then
			impulseWeaponID[i].impulseMaxDepth = -tonumber(wd.customParams.impulsemaxdepth)
			impulseWeaponID[i].impulseDepthMult = -tonumber(wd.customParams.impulsedepthmult)
		end
	end
end
...
function gadget:UnitPreDamaged(unitID, unitDefID, unitTeam, damage, paralyzer, weaponDefID, attackerID, attackerDefID, attackerTeam)
	...
	if weaponDefID and attackerID and impulseWeaponID[weaponDefID] and Spring.ValidUnitID(attackerID) then
		...
		--Spring.Echo("Normal Damage: " .. tostring(defData.normalDamage))
		if defData.normalDamage then
			--Spring.Echo("Normal damage: " .. damage)
			return damage
		else
			return 0
		end
