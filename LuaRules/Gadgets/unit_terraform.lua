...
local function SetTooltip(unitID, spent, estimatedCost)
	spSetUnitRulesParam(unitID, "terraform_spent", spent, ALLIED_TABLE)
	...
	spSetUnitRulesParam(unitID, "terraform_estimate", estimatedCost, ALLIED_TABLE)
end
...
local function IsNearbyEnemy(unitID, checkDist)
	if terraformUnit[unitID].restore then
		return
	end
	local x, y, z = spGetUnitPosition(unitID)
	local nearbyUnits = spGetUnitsInSphere(x, y, z, checkDist)
	if #nearbyUnits <= 1 then -- only the unit itself is nearby.
		return
	else
		local teamID = spGetUnitTeam(unitID)
		local allyTeamID = spGetUnitAllyTeam(unitID)
		for i = 1, #nearbyUnits do
			local potentialEnemy = nearbyUnits[i]
			if not spAreTeamsAllied(teamID, spGetUnitTeam(potentialEnemy)) then
				local unitDefID = spGetUnitDefID(potentialEnemy)
				if unitDefID ~= terraunitDefID and UnitDefs[unitDefID].isGroundUnit and not spGetUnitIsCloaked(potentialEnemy) then
...
local function CheckNearbyEnemy(unitID)
	...
	local nearbyEnemyID = IsNearbyEnemy(unitID, terraformUnit[unitID].enemyCheckDist, true)
...
local function TerraformWall(terraform_type, mPoint, mPoints, terraformHeight, unit, units, team, volumeSelection, shift, commandX, commandZ, commandTag, disableForceCompletion, noDecay)
	...
	for i = 1,n-1 do
		...
		elseif terraform_type == 5 then --restore
			for j = 1, segment[i].points do
				if not segment[i].area[segment[i].point[j].x] then
					segment[i].area[segment[i].point[j].x] = {}
				end
...
local function TerraformArea(terraform_type, mPoint, mPoints, terraformHeight, unit, units, team, volumeSelection, shift, commandX, commandZ, commandTag, disableForceCompletion, noDecay)
	...
	for i = 1,n-1 do
		...
		elseif terraform_type == 5 then -- retore
			for j = 1, segment[i].points do
				if not segment[i].area[segment[i].point[j].x] then
					segment[i].area[segment[i].point[j].x] = {}
				end
