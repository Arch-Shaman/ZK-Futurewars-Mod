...
local function SpawnUnit(spawnData)
	...
	if spawnDef.feature then
		...
		if spawnDef.expire then
			spSetUnitRulesParam(unitID, "lifetime_total", spawnDef.expire, ALLIED_ACCESS)
			spSetUnitRulesParam(unitID, "lifetime_expiry", spGetGameFrame() + spawnDef.expire, ALLIED_ACCESS)
			GG.UnitCallLater(unitID, spawnDef.expire, spDestroyUnit, true)
		end
