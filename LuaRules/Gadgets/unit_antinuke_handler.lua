...
function gadget:AllowWeaponInterceptTarget(interceptorUnitID, interceptorWeaponNum, targetProjectileID)
	...
	local px, height, pz = Spring.GetProjectilePosition(targetProjectileID)
	height = height - Spring.GetGroundHeight(px, pz) -- NOTE: 4000 AGL is "upper atmosphere" -- oblivion flies at 6000 as a "it's in space" thing.
	local targetType, targetData = Spring.GetProjectileTarget(targetProjectileID)
	if targetType ~= 103 and height < 4000 then -- ASCII 'g' for ground
	...
	if leftInt < tx and rightInt > 0 and height < 4000 then
		return true
	end
	return false
end
