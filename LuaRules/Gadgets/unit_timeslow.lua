...
local function dealSlowToUnit(unitID, damage, overslow, attackerTeam)
	if not slowedUnits[unitID] then
		slowedUnits[unitID] = {
			slowDamage = 0,
			degradeTimer = DEGRADE_TIMER,
	...
	if overslow then
		slowedUnits[unitID].extraSlowBound = math.max(slowedUnits[unitID].extraSlowBound or 0, overslow)
	end
...
GG.dealSlowToUnit = dealSlowToUnit
