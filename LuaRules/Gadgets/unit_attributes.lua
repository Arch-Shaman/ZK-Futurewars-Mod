...
local function UpdateReloadSpeed(unitID, unitDefID, weaponMods, speedFactor, gameFrame)
	...
	for i = 1, state.weaponCount do
		...
		if speedFactor <= 0 then
			...
			local baseReloadTime = spGetUnitRulesParam(unitID, i .. "_basereload") or w.reload
			local moddedSpeed = ((weaponMods and weaponMods[i] and weaponMods[i].reloadMult) or 1)*speedFactor
			local newReload = baseReloadTime/moddedSpeed
			local nextReload = gameFrame+(reloadState-gameFrame)*newReload/reloadTime
			local burstRate = spGetUnitRulesParam(unitID, i .. "_baseburstrate") or w.burstRate
			...
			if w.burstRate then
				spSetUnitWeaponState(unitID, i, {reloadTime = newReload + HALF_FRAME, reloadState = nextReload, burstRate = burstRate/moddedSpeed + HALF_FRAME})
			else
