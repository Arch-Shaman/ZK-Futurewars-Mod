		...
		for i = 1, unitIndex.count do
			...
			if unit.progress >= unit.defs.cost then
				...
				if newId then
					...
					if currentcmd[1] and currentcmd[1].id == CMD_GREYGOO then
						spGiveOrderToUnit(newId, CMD_GREYGOO, currentcmd[1].params, 0) -- makes it go around to other wrecks too (minor optimization)
