-- lightningplosion
-- lightningplosion160AoE
-- lightningplosion_yellowbolts
-- zeusmuzzle
-- panther_spark
-- lightningplosion_bluebolts1
-- lightningplosion_stormbolt
-- zeusgroundflash
-- lightningplosion_smallbolts

local circleString = "r%.5f y10 -1 x10x10 y10 %d a10 y10      r6.283 y11 -3.1415 a11 y11 -0.5x11x11         y0 0.0417x11x11x11x11 y1 -0.00139x11x11x11x11x11x11 y2 0.0000248015x11x11x11x11x11x11x11x11 y3 -0.000000275573x11x11x11x11x11x11x11x11x11x11 y4 0.00000000208768x11x11x11x11x11x11x11x11x11x11x11x11 y5 1 a0 a1 a2 a3 a4 a5 x10, %d, -0.1667x11x11x11 y0 0.00833x11x11x11x11x11 y1 -0.000198412x11x11x11x11x11x11x11 y2 0.00000275573192x11x11x11x11x11x11x11x11x11 y3 -0.00000002505210838x11x11x11x11x11x11x11x11x11x11x11 y4 0 a11 a0 a1 a2 a3 a4 x10"

local function GetCircle(radius, yPos)
	radius = radius
	local weightedRadius = radius^(1/2)
	return circleString:format(weightedRadius, radius, yPos)
end

return {
	["lightningplosion"] = {
		bluebolts1 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = 3,
				explosiongenerator = [[custom:LIGHTNINGPLOSION_BLUEBOLTS1]],
				pos                = [[0, 0, 0]],
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 20,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[10 r200]],
				explosiongenerator = [[custom:LIGHTNINGPLOSION_STORMBOLT]],
				pos                = GetCircle(100, 1),
			},
		},
		groundflash = {
			circlealpha        = 1,
			circlegrowth       = 0,
			flashalpha         = 0.3,
			flashsize          = 86,
			ttl                = 12,
			color = {
				[1]  = 0.5,
				[2]  = 0.5,
				[3]  = 1,
			},
		},
		pikes = {
			air                = true,
			class              = [[explspike]],
			count              = 15,
			ground             = true,
			water              = true,
			properties = {
				alpha              = 0.8,
				alphadecay         = 0.1,
				color              = [[0.5,0.5,1]],
				dir                = [[-15 r30,-15 r30,-15 r30]],
				length             = 30,
				width              = 5,
			},
		},
		yellowbolts = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = 0,
				explosiongenerator = [[custom:LIGHTNINGPLOSION_YELLOWBOLTS]],
				pos                = [[0, 0, 0]],
			},
		},
	},
  
	["lightningplosion_nopost"] = {
		bluebolts1 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = 3,
				explosiongenerator = [[custom:LIGHTNINGPLOSION_BLUEBOLTS1]],
				pos                = [[0, 0, 0]],
			},
		},
		sparks = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
			   airdrag            = 0.97,
			   colormap           = [[0.2 0.2 1 0.01   0.4 0.4 1 0.01   0.0 0.0 0 0.01]],
			   directional        = true,
			   emitrot            = 0,
			   emitrotspread      = 60,
			   emitvector         = [[0, 1, 0]],
			   gravity            = [[0, 0, 0]],
			   numparticles       = 40,
			   particlelife       = 20,
			   particlelifespread = 5,
			   particlesize       = 5,
			   particlesizespread = 0,
			   particlespeed      = 5,
			   particlespeedspread = 0,
			   pos                = [[0, 0, 0]],
			   sizegrowth         = 0,
			   sizemod            = 1.0,
			   texture            = [[spark]],
			},
		},
		groundflash = {
			circlealpha        = 1,
			circlegrowth       = 0,
			flashalpha         = 0.3,
			flashsize          = 86,
			ttl                = 12,
			color = {
				[1]  = 0.5,
				[2]  = 0.5,
				[3]  = 1,
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 5,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[2 r2]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered]],
				pos                = GetCircle(30, 3),
			},
		},
		pikes = {
			air                = true,
			class              = [[explspike]],
			count              = 15,
			ground             = true,
			water              = true,
			properties = {
				alpha              = 0.8,
				alphadecay         = 0.1,
				color              = [[0.5,0.5,1]],
				dir                = [[-15 r30,-15 r30,-15 r30]],
				length             = 30,
				width              = 5,
			},
		},
		yellowbolts = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = 0,
				explosiongenerator = [[custom:LIGHTNINGPLOSION_YELLOWBOLTS]],
				pos                = [[0, 0, 0]],
			},
		},
	},


	["lightningplosion160AoE"] = {
		electricsblue = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 3,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[1 r10]],
				explosiongenerator = [[custom:YELLOW_LIGHTNING_BOMB_BLUEBOLTS]],
				pos                = [[0, 0, 0]],
			},
		},
		electricsyellow = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 3,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[1 r20]],
				explosiongenerator = [[custom:YELLOW_LIGHTNING_STORMBOLT]],
				pos                = GetCircle(100, 1),
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 20,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[10 r200]],
				explosiongenerator = [[custom:LIGHTNINGPLOSION_STORMBOLT]],
				pos                = GetCircle(100, 1),
			},
		},
		groundflash = {
			circlealpha        = 1,
			circlegrowth       = 0,
			flashalpha         = 0.4,
			flashsize          = 170,
			ttl                = 32,
			color = {
				[1]  = 0.5,
				[2]  = 0.5,
				[3]  = 1,
			},
		},
		pikes = {
			air                = true,
			class              = [[explspike]],
			count              = 15,
			ground             = true,
			water              = true,
			properties = {
				alpha              = 0.8,
				alphadecay         = 0.1,
				color              = [[0.5,0.5,1]],
				dir                = [[-15 r30,-15 r30,-15 r30]],
				length             = 30,
				width              = 5,
			},
		},
		yellowbolts = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = 0,
				explosiongenerator = [[custom:YELLOW_LIGHTNING_YELLOWBOLTS]],
				pos                = [[0, 0, 0]],
			},
		},
	},

	["lightningplosion128AoE"] = {
		electricsblue = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 3,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[1 r10]],
				explosiongenerator = [[custom:YELLOW_LIGHTNING_BOMB_BLUEBOLTS]],
				pos                = [[0, 0, 0]],
			},
		},
		electricsyellow = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 3,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[1 r20]],
				explosiongenerator = [[custom:YELLOW_LIGHTNING_STORMBOLT]],
				pos                = GetCircle(100, 1),
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 20,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[10 r200]],
				explosiongenerator = [[custom:LIGHTNINGPLOSION_STORMBOLT]],
				pos                = GetCircle(100, 1),
			},
		},
		groundflash = {
			circlealpha        = 1,
			circlegrowth       = 0,
			flashalpha         = 0.4,
			flashsize          = 136,
			ttl                = 32,
			color = {
				[1]  = 0.5,
				[2]  = 0.5,
				[3]  = 1,
			},
		},
		pikes = {
			air                = true,
			class              = [[explspike]],
			count              = 15,
			ground             = true,
			water              = true,
			properties = {
				alpha              = 0.8,
				alphadecay         = 0.1,
				color              = [[0.5,0.5,1]],
				dir                = [[-15 r30,-15 r30,-15 r30]],
				length             = 30,
				width              = 5,
			},
		},
		yellowbolts = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = 0,
				explosiongenerator = [[custom:YELLOW_LIGHTNING_YELLOWBOLTS]],
				pos                = [[0, 0, 0]],
			},
		},
	},

	["lightningplosion_yellowbolts"] = {
		["electric thingies"] = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.5  0.5 1 0.01  0.5 0.5 1 0.01 1 1 0 0.01  0.5 0.5 1 0.01   0 0 0 0.01]],
				directional        = true,
				emitrot            = 0,
				emitrotspread      = 180,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = 5,
				particlelife       = 2,
				particlelifespread = 5,
				particlesize       = 40,
				particlesizespread = 0,
				particlespeed      = 2,
				particlespeedspread = 0,
				pos                = [[0, 1.0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[whitelightb]],
			},
		},
		sparks = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.97,
				colormap           = [[0.2 0.2 1 0.01   0.4 0.4 1 0.01   0.0 0.0 0 0.01]],
				directional        = true,
				emitrot            = 0,
				emitrotspread      = 60,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = 40,
				particlelife       = 20,
				particlelifespread = 5,
				particlesize       = 5,
				particlesizespread = 0,
				particlespeed      = 5,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[spark]],
			},
		},
	},

	["zeusmuzzle"] = {
		usedefaultexplosions = false,
		lightball = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 20,
				particlelifespread = 0,
				particlesize       = 15,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb3]],
				useairlos          = false,
			},
		},
		lightball2 = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0 0 0 0.01 0 0 0 0.01 0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 20,
				particlelifespread = 0,
				particlesize       = 15,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb4]],
				useairlos          = false,
			},
		},
		lightring = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.3 0.3 0.7 0.01  0.7 0.7 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 20,
				particlelifespread = 0,
				particlesize       = 1,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 1,
				sizemod            = 1.0,
				texture            = [[lightring]],
				useairlos          = false,
			},
		},
	},
  
	["panther_spark"] = {
		lightningballs = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 1,
				colormap           = [[0 0 0 0.01 1 1 1 0.01 0 0 0 0.01]],
				directional        = true,
				emitrot            = 80,
				emitrotspread      = 0,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 3,
				particlelifespread = 0,
				particlesize       = 2,
				particlesizespread = 2,
				particlespeed      = 0.01,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb]],
				useairlos          = false,
			},
		},
	},
  
	["antagonist_spark"] = {
		lightball = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 15,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.75,
				texture            = [[lightb3]],
				useairlos          = false,
			},
		},
		lightball2 = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0 0 0 0.01 0 0 0 0.01 0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 15,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.75,
				texture            = [[lightb4]],
				useairlos          = false,
			},
		},
		lightring = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.3 0.3 0.7 0.01  0.7 0.7 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 1,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 1,
				sizemod            = 0.75,
				texture            = [[lightring]],
				useairlos          = false,
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[0 r2]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered]],
				pos                = GetCircle(2, 0),
			},
		},
	},
	["antagonist_spark_bigger"] = {
		lightball = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 40,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.75,
				texture            = [[lightb3]],
				useairlos          = false,
			},
		},
		lightball2 = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0 0 0 0.01 0 0 0 0.01 0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 40,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.75,
				texture            = [[lightb4]],
				useairlos          = false,
			},
		},
		lightring = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.3 0.3 0.7 0.01  0.7 0.7 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 1,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 1,
				sizemod            = 0.75,
				texture            = [[lightring]],
				useairlos          = false,
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[0 r2]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered]],
				pos                = GetCircle(4, 0),
			},
		},
	},
	["artillery_spark"] = {
		usedefaultexplosions = false,
		lightball = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 3,
				particlelifespread = 0,
				particlesize       = 15,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb3]],
				useairlos          = false,
			},
		},
		lightball2 = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0 0 0 0.01 0 0 0 0.01 0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 3,
				particlelifespread = 0,
				particlesize       = 15,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb4]],
				useairlos          = false,
			},
		},
		largeflash = {
			air                = true,
			class              = [[CBitmapMuzzleFlame]],
			count              = 1,
			ground             = true,
			underwater         = 1,
			water              = true,
			properties = {
				colormap           = [[0.3 0.3 0.7 0.05 0.2 0.2 0.45 0.01 0 0 0 0.01]],
				dir                = [[dir]],
				frontoffset        = 0,
				fronttexture       = [[muzzlefront]],
				length             = -45,
				sidetexture        = [[muzzleside]],
				size               = -8,
				sizegrowth         = 0.75,
				ttl                = 8,
			},
		},
		lightring = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.3 0.3 0.7 0.01  0.7 0.7 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 1,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 3,
				particlelifespread = 0,
				particlesize       = 1,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 1,
				sizemod            = 1.0,
				texture            = [[lightring]],
				useairlos          = false,
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[0 r2]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered]],
				pos                = GetCircle(3, 0),
			},
		},
	},
	["comm_shockhit"] = {
		usedefaultexplosions = false,
		groundflash = {
			alwaysvisible      = true,
			circlealpha        = 0.4,
			circlegrowth       = 7,
			flashalpha         = 0.5,
			flashsize          = 20,
			ttl                = 14,
			color = {
				[1]  = 0.3,
				[2]  = 0.3,
				[3]  = 0.7,
			},
		},
		sphere = {
			air                = true,
			class              = [[CSpherePartSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			underwater 		 = true,
			properties = {
				alpha              = 0.4,
				color              = [[0.3,0.3,0.7]],
				expansionspeed     = 2,
				ttl                = 10,
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[1]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_alwaysshow]],
				pos                = GetCircle(5, 0),
			},
		},
		electricstorm2 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[4]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_alwaysshow]],
				pos                = GetCircle(10, 0),
			},
		},
		electricstorm3 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[8]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_alwaysshow]],
				pos                = GetCircle(15, 0),
			},
		},
	},
	["drp_blue_trail"] = {
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[0]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_trail]],
				pos                = GetCircle(10, 0),
			},
		},
		electricstorm2 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[1]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_trail]],
				pos                = GetCircle(15, 0),
			},
		},
		electricstorm3 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[2]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_trail]],
				pos                = GetCircle(20, 0),
			},
		},
	},
	["hammer_artillery_hit"] = {
		usedefaultexplosions = false,
		groundflash = {
			alwaysvisible      = true,
			circlealpha        = 0.4,
			circlegrowth       = 7,
			flashalpha         = 0.5,
			flashsize          = 60,
			ttl                = 14,
			color = {
				[1]  = 0.3,
				[2]  = 0.3,
				[3]  = 0.7,
			},
		},
		sphere = {
			air                = true,
			class              = [[CSpherePartSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			underwater 		 = true,
			properties = {
				alpha              = 0.4,
				color              = [[0.3,0.3,0.7]],
				expansionspeed     = 6,
				ttl                = 8,
			},
		},
		electricstorm = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[1]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_alwaysshow]],
				pos                = GetCircle(20, 0),
			},
		},
		electricstorm2 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[4]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_alwaysshow]],
				pos                = GetCircle(40, 0),
			},
		},
		electricstorm3 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[8]],
				explosiongenerator = [[custom:lightningplosion_smallbolts_centered_alwaysshow]],
				pos                = GetCircle(60, 0),
			},
		},
	},
	["lightningballframe1"] = {
		lightball = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 80,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 12,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.8,
				texture            = [[lightb3]],
				useairlos          = true,
			},
		},
	},
	["lightningballframe2"] = {
		lightball2 = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0 0 0 0.01 0 0 0 0.01 0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 80,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 12,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.5,
				texture            = [[lightb4]],
				useairlos          = true,
			},
		},
	},
	["lightningballanimated"] = {
		useAirLos = true,
		frame1 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[0]],
				explosiongenerator = [[custom:lightningballframe1]],
			},
		},
		frame2 = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = [[2]],
				explosiongenerator = [[custom:lightningballframe2]],
			},
		},
	},
	["artillery_spark_small"] = {
		lightball = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 180,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 15,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.75,
				texture            = [[lightb3]],
				useairlos          = false,
			},
		},
		lightball2 = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0 0 0 0.01 0 0 0 0.01 0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 180,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 15,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.75,
				texture            = [[lightb4]],
				useairlos          = false,
			},
		},
	},
	
	["artillery_spark_small_commander"] = {
		lightball = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 180,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 10,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.75,
				texture            = [[lightb3]],
				useairlos          = false,
			},
		},
		lightball2 = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.95,
				colormap           = [[0 0 0 0.01 0 0 0 0.01 0.7 0.7 0.7 0.01  1 1 1 0.01  0 0 0 0.01]],
				directional        = false,
				emitrot            = 0,
				emitrotspread      = 180,
				emitvector         = [[dir]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 2,
				particlelifespread = 0,
				particlesize       = 10,
				particlesizespread = 0,
				particlespeed      = 0,
				particlespeedspread = 0,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 0.75,
				texture            = [[lightb4]],
				useairlos          = false,
			},
		},
	},

	["lightningplosion_bluebolts1"] = {
		["electric thingies2"] = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.1,
				colormap           = [[1 1 1 0.01  1 1 1 0.01   1 1 1 0.01  1 1 1 0.01  1 1 1 0.01 0 0 0 0.01]],
				directional        = true,
				emitrot            = 0,
				emitrotspread      = 80,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = 10,
				particlelife       = 8,
				particlelifespread = 4,
				particlesize       = 15,
				particlesizespread = 15,
				particlespeed      = 20,
				particlespeedspread = 20,
				pos                = [[0, 1.0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb]],
			},
		},
	},

	["lightningplosion_stormbolt"] = {
		groundflash = {
			circlealpha        = 1,
			circlegrowth       = 0,
			flashalpha         = 0.3,
			flashsize          = 46,
			ttl                = 3,
			color = {
				[1]  = 0.5,
				[2]  = 0.5,
				[3]  = 1,
			},
		},
		lightningballs = {

			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.1,
				colormap           = [[1 1 1 0.01  1 1 1 0.01   1 1 1 0.01  1 1 1 0.01  1 1 1 0.01 0 0 0 0.01]],
				directional        = true,
				emitrot            = 20,
				emitrotspread      = 20,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 8,
				particlelifespread = 4,
				particlesize       = 15,
				particlesizespread = 15,
				particlespeed      = 1,
				particlespeedspread = 1,
				pos                = [[0, 1.0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb]],
			},
		},
	},

	["zeusgroundflash"] = {
		groundflash = {
			circlealpha        = 1,
			circlegrowth       = 0,
			flashalpha         = 0.3,
			flashsize          = 46,
			ttl                = 12,
			color = {
				[1]  = 0.5,
				[2]  = 0.5,
				[3]  = 1,
			},
		},
	},

	["lightningplosion_smallbolts"] = {
		["electric thingies2"] = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.1,
				colormap           = [[1 1 1 0.01  1 1 1 0.01   1 1 1 0.01  1 1 1 0.01  1 1 1 0.01 0 0 0 0.01]],
				directional        = true,
				emitrot            = 0,
				emitrotspread      = 80,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 8,
				particlelifespread = 4,
				particlesize       = 15,
				particlesizespread = 15,
				particlespeed      = 20,
				particlespeedspread = 20,
				pos                = [[0, 1.0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb]],
			},
		},
	},
  
	["lightningplosion_smallbolts_centered"] = {
		["electric thingies2"] = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.1,
				colormap           = [[1 1 1 0.01  1 1 1 0.01   1 1 1 0.01  1 1 1 0.01  1 1 1 0.01 0 0 0 0.01]],
				directional        = true,
				emitrot            = 0,
				emitrotspread      = 80,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = [[r3 -1]],
				particlelife       = 3,
				particlelifespread = 4,
				particlesize       = 15,
				particlesizespread = 15,
				particlespeed      = 20,
				particlespeedspread = 20,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb]],
			},
		},
	},
	
	["lightningplosion_smallbolts_centered_alwaysshow"] = {
		["electric thingies2"] = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.1,
				colormap           = [[1 1 1 0.01  1 1 1 0.01   1 1 1 0.01  1 1 1 0.01  1 1 1 0.01 0 0 0 0.01]],
				directional        = true,
				emitrot            = 0,
				emitrotspread      = 80,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = [[r2 1]],
				particlelife       = 3,
				particlelifespread = 4,
				particlesize       = 15,
				particlesizespread = 15,
				particlespeed      = 20,
				particlespeedspread = 20,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb]],
			},
		},
	},
	
	["lightningplosion_smallbolts_centered_trail"] = {
		["electric thingies2"] = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.1,
				colormap           = [[1 1 1 0.01  1 1 1 0.01   1 1 1 0.01  1 1 1 0.01  1 1 1 0.01 0 0 0 0.01]],
				directional        = true,
				emitrot            = 0,
				emitrotspread      = 80,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = [[r2 1]],
				particlelife       = 2,
				particlelifespread = 1,
				particlesize       = 15,
				particlesizespread = 15,
				particlespeed      = 20,
				particlespeedspread = 20,
				pos                = [[0, 0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb]],
			},
		},
	},
	

	-- muzzle and groundflash as a single reference
	["zeus_fire_fx"] = {
		muzzle = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = 0,
				explosiongenerator = [[custom:zeusmuzzle]],
				pos                = [[0, 0, 0]],
				dir                = [[dir]],
			},
		},
		dust = {
			air                = true,
			class              = [[CExpGenSpawner]],
			count              = 1,
			ground             = true,
			water              = true,
			properties = {
				delay              = 0,
				explosiongenerator = [[custom:zeusgroundflash]],
				pos                = [[0, 0, 0]],
				dir                = [[dir]],
			},
		},
	},

	["lightningplosion_smallbolts_purple"] = {
		["electric thingies2"] = {
			air                = true,
			class              = [[CSimpleParticleSystem]],
			count              = 2,
			ground             = true,
			water              = true,
			properties = {
				airdrag            = 0.1,
				colormap           = [[1 1 1 0.01  1 1 1 0.01   1 1 1 0.01  1 1 1 0.01  1 1 1 0.01 0 0 0 0.01]],
				directional        = true,
				emitrot            = 0,
				emitrotspread      = 80,
				emitvector         = [[0, 1, 0]],
				gravity            = [[0, 0, 0]],
				numparticles       = 1,
				particlelife       = 8,
				particlelifespread = 4,
				particlesize       = 8,
				particlesizespread = 8,
				particlespeed      = 10,
				particlespeedspread = 10,
				pos                = [[0, 1.0, 0]],
				sizegrowth         = 0,
				sizemod            = 1.0,
				texture            = [[lightb_purple]],
			},
		},
	},
}
