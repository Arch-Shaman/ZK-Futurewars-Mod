...
local function AddUnit(unitID)
	if not drawUnitID[unitID] then
		if spectating or spGetUnitAllyTeam(unitID) == myAllyTeamID then
			...
			droneSetTargetUnit[unitID] = (unitDefID and carrierDefs[unitDefID] or spGetUnitRulesParam(unitID, "comm_level")) and true			
		end
	end
end
...
local function getDroneTargetPosition(unitID)
	if not droneSetTargetUnit[unitID] then
		return nil
	end
	local target_type = spGetUnitRulesParam(unitID,"drone_target_type") or TARGET_NONE
	
	local tx, ty, tz
	
	if target_type == TARGET_GROUND then
		tx = spGetUnitRulesParam(unitID, "drone_target_x")
		ty = spGetUnitRulesParam(unitID, "drone_target_y")
		tz = spGetUnitRulesParam(unitID, "drone_target_z")
	elseif target_type == TARGET_UNIT then
		local targetID = spGetUnitRulesParam(unitID, "drone_target_id")
		if targetID and targetID ~= 0 and Spring.ValidUnitID(targetID) then
			_, _, _, tx, ty, tz = spGetUnitPosition(targetID, true)
		else
			return nil
...
local function drawUnitCommands(unitID)
	...
	local dtx, dty, dtz = getDroneTargetPosition(unitID)
	if dtx then
		local _,_,_,x,y,z = spGetUnitPosition(unitID,true)
		glColor(0, 0.8, 1, setTargetAlpha)
		glBeginEnd(GL.LINES,
			function()
				glVertex(x,y,z)
				glVertex(dtx,dty,dtz)
			end)
	end
end
