...
do
	...
	for unitName, conf in pairs(defs) do
...
RedoUnitList = function()
	...
	for _, def in pairs(unitConfig) do
		...
		elseif def.class == SHIELD then
			...
			def.wantedSpec = false
		elseif def.class == CLOAK then
			def.wantedAlly = options.allycloak.value
			def.wantedEnemy = options.enemycloak.value
			def.wantedSpec = options.speccloak.value
		end
	end
...
local function SetupChiliStuff()
	...
	pics.cloak  = Image:New { x = 0, y = 24*6, file = 'LuaUI/Images/defense_ranges/cloak.png'  }
