...
local tooltips = {
	...
	retreatshield = {
		[-1] = "Inherit from factory",
		[0] = "Never Retreat",
		[1] = "Retreat at 30% shield",
		[2] = "Retreat at 50% shield",
		[3] = "Retreat at 80% shield",
	},
	auto_call_transport = {
		[-1] = "Inherit from factory",
		[0] = "Disabled",
	...
	if (ud.canMove or ud.isFactory) then
		...
		for w = 1, #weapons do
			...
			if WeaponDefs[wep].shieldPower and WeaponDefs[wep].shieldPower > 0 then
				...
				options_order[#options_order+1] = defName .. "_retreatpercent_shield"
				break
			end
		end
	end
...
function widget:UnitCreated(unitID, unitDefID, unitTeam, builderID)
	...
	if unitAlreadyAdded[name] then
		...
		if retreatshield then
			if retreatshield == 0 then
				orderArray[#orderArray + 1] = {CMD_RETREATSHIELD, {0}, CMD.OPT_SHIFT + CMD.OPT_RIGHT}  -- to set retreat to 0, "right" option must be used
			else
				orderArray[#orderArray + 1] = {CMD_RETREATSHIELD, {retreatshield}, CMD.OPT_SHIFT}
			end
		end
