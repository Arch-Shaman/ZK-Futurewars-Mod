...
local function OverDriveThread()
	...
	GG.PlayFogHiddenSound("sounds/weapon/laser/death_laser_charge.wav", 25, px, py, pz) -- TODO: make new sound.
	overdrive = true
	Spring.SetUnitRulesParam(unitID, "selfMoveSpeedChange", 1.25)
	GG.UpdateUnitAttributes(unitID)
	Sleep(10000)
	overdrive = false
	Spring.SetUnitRulesParam(unitID, "selfMoveSpeedChange", 0.66)
	GG.UpdateUnitAttributes(unitID)
	postoverdrive = true
	Sleep(10000)
	postoverdrive = false
...
function script.BlockShot(num)
	if num == 1 then
		return overdrive or postoverdrive
	elseif num == 2 then
		return (not overdrive) or postoverdrive
	end
end
