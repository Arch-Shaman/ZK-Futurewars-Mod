...
local function FireCycle()
	reloaded = false
	Signal(SIG_CYCLE)
	SetSignalMask(SIG_CYCLE)
	Sleep(0.7 * 30 * (1000 / Game.gameSpeed)) -- wait for beam to end.
	--GG.DelegateOrder(unitID, CMD_UNIT_CANCEL_TARGET, {}, 0)
	Close()
	local reloadSpeedMult = Spring.GetUnitRulesParam(unitID, "totalReloadSpeedChange") or 1
	Sleep((5.3 * 30 * reloadSpeedMult) * (1000 / Game.gameSpeed))
	reloaded = true
end
...
function script.AimWeapon(num, heading, pitch)
	...
	if not reloaded then return false end
	Signal(SIG_AIM)
	SetSignalMask(SIG_AIM)
...
function script.BlockShot(num, targetID)
	return GG.Script.OverkillPreventionCheck(unitID, targetID, 820.1, 820, 32, 0.2)
end
