...
local DroneInner = piece('DroneInner');

local LandingAft1 = piece('LandingAft1');
local LandingAft2 = piece('LandingAft2');
local LandingAft3 = piece('LandingAft3');
local LandingAft4 = piece('LandingAft4');
local LandingFore1 = piece('LandingFore1');
local LandingFore2 = piece('LandingFore2');
local LandingFore3 = piece('LandingFore3');
local LandingFore4 = piece('LandingFore4');
...
droneBays[DroneInner] = {
	slider=BayInnerSlider,
	hatch=BayInnerHatch,
	direction=-1,
}
...
function script.Create()
	...
	for bay, data in pairs(droneBays) do
		Move(bay, x_axis, 16 * data.direction)
	end
...
function script.QueryLandingPads()
	return {LandingFore1, LandingFore2, LandingFore3, LandingFore4, LandingAft1, LandingAft2, LandingAft3, LandingAft4}
end
...
function Carrier_droneStarted(piece)
	if(piece) then
		...
		if(bay) then
			Signal("bay"..piece)
			Move(bay.slider, x_axis, -7 * bay.direction, 3);
			Turn(bay.hatch,  z_axis, math.rad(-100) * bay.direction, math.rad(60));
		end
	end
end
