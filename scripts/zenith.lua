...
local function TransformMeteor(weaponDefID, proID, meteorTeamID, meteorOwnerID, x, y, z)
	...
	if x then
		...
		GG.ForceCruiseUpdate(newProID, x, y, z)
	else
		local x, y, z = Spring.GetProjectilePosition(newProID)
...
local function DropSingleMeteor(index)
	...
	if Spring.GetProjectileDefID(proID) == floatWeaponDefID then
		TransformMeteor(uncontrolWeaponDefID, proID, gaiaTeam, unitID)
	end
end
...
local function LoseControlOfMeteors()
	...
	for i = 1, projectileCount do
		...
		if Spring.GetProjectileDefID(proID) == floatWeaponDefID then
			...
			projectiles[i] = TransformMeteor(uncontrolWeaponDefID, proID, gaiaTeam, unitID)
		end
	end
	...
	for i = 1, projectileCount do
		...
		if Spring.GetProjectileDefID(proID) == uncontrolWeaponDefID then
			...
			if ttl > 0 then
				...
				projectiles[i] = TransformMeteor(floatWeaponDefID, proID, gaiaTeam, unitID, ux + hoverPos[1], uy + HOVER_HEIGHT, uz + hoverPos[2])
			end
		end
	end
	Spring.SetUnitRulesParam(unitID, "meteorsControlled", tooltipProjectileCount, INLOS_ACCESS)
...
local function SpawnProjectileThread()
	...
	while true do
		...
		--reloadMult = reloadMult * (Spring.GetUnitRulesParam(unitID,"superweapon_mult") or 0)
		...
		ready = GG.FireControl.CanFireWeapon(unitID, 2)
		while IsDisabled() do
			UpdateEnabled(false)
			ready = GG.FireControl.CanFireWeapon(unitID, 2)
			Sleep(33)
...
function script.AimWeapon(num, heading, pitch)
	return (num ~= 2) and ((spGetUnitRulesParam(unitID, "lowpower") or 0) == 0) and GG.FireControl.CanFireWeapon(unitID, num) and not firstRun
end
