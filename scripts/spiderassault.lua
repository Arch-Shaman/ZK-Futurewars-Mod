...
function script.FireWeapon(num)
	StartThread(recoil, num)
end

function script.BlockShot(num, targetID)
	if Spring.ValidUnitID(targetID) then
		local distMult = (Spring.GetUnitSeparation(unitID, targetID) or 0)/410
		return GG.OverkillPrevention_CheckBlock(unitID, targetID, 200, 30 * distMult, false, false, true)
	end
	return false
end
