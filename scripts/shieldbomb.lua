...
--[[local function ShieldControlThread()
	local shieldhp = 0
	while true do
		if burrowed and shieldstate then
			_, shieldhp = Spring.GetUnitShieldState(unitID, 1)
			shieldstate = false
			Spring.SetUnitRulesParam(unitID, "shielddisabled", 1, PRIVATE)
		elseif not shieldstate and not burrowed then
			shieldstate = true
			Spring.SetUnitRulesParam(unitID, "shielddisabled", nil)
			Spring.SetUnitShieldState(unitID, 1, 1, shieldhp)
		end
		Sleep(200)
	end
end]]
