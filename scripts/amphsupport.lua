...
local RELOAD_TIME = tonumber(WeaponDefNames["amphsupport_cannon"].customParams.script_reload) * gameSpeed
...
local function WeaponRangeUpdate()
	while true do
		...
		if height < -20 then
			Spring.SetUnitWeaponState(unitID, 2, {range = 1100 - height})
			Spring.SetUnitMaxRange(unitID, 1100 - height)
		else
			Spring.SetUnitWeaponState(unitID, 2, {range = 1100})
			Spring.SetUnitMaxRange(unitID, 1100)
		end
		Sleep(500)
	end
end
...
function script.Create()
	...
	scriptReload.SetupScriptReload(6, RELOAD_TIME)
end
...
local function recoil(num)
	if num%2 == 0 then
		EmitSfx(lflare, 1024)
		EmitSfx(lflare, 1025)
		Move(lbarrel, z_axis, -6, 20)
		Sleep(200)
		Move(lbarrel, z_axis, GUN_DEPLOY_DIST, 17.24)
	else
		EmitSfx(rflare, 1024)
		EmitSfx(rflare, 1025)
		Move(rbarrel, z_axis, -6, 20)
		Sleep(200)
		Move(rbarrel, z_axis, GUN_DEPLOY_DIST, 17.24)
	end
end

local function Reload(num)
	scriptReload.GunStartReload(num)
	ammo[num].loaded = false
	SleepAndUpdateReload(num, RELOAD_TIME)
	ammo[num].loaded = true
...
function script.Shot(num)
	StartThread(Reload, gun)
	StartThread(recoil, gun)
	gun = gun % 6 + 1
end

function script.BlockShot(num, targetID)
	if not ammo[gun].loaded then
		return true
	end
	if Spring.ValidUnitID(targetID) then
		local distMult = (Spring.GetUnitSeparation(unitID, targetID) or 0)/1100
		-- Reduced damage because it has a chance of missing.
		return GG.OverkillPrevention_CheckBlock(unitID, targetID, 400.01, 110 * distMult, false, false, true)
	end
	return false
end
...
function script.Killed(recentDamage, maxHealth)
	...
	if severity <= 0.5 then
		...
		Explode(lthigh, SFX.FALL + SFX.FIRE + SFX.SMOKE + SFX.EXPLODE_ON_HIT)
		Explode(rcalf, SFX.FALL + SFX.FIRE + SFX.SMOKE + SFX.EXPLODE_ON_HIT)
		Explode(lcalf, SFX.FALL + SFX.FIRE + SFX.SMOKE + SFX.EXPLODE_ON_HIT)
		Explode(rfoot, SFX.FALL + SFX.FIRE + SFX.SMOKE + SFX.EXPLODE_ON_HIT)
		Explode(lfoot, SFX.FALL + SFX.FIRE + SFX.SMOKE + SFX.EXPLODE_ON_HIT)
		return 1
	end
