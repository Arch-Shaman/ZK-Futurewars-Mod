...
local gunPieces = {
	[1] = {flare = lflare, recoil = lbarrel2},
	[2] = {flare = rflare, recoil = rbarrel2},
}
...
function script.BlockShot(num, targetID)
	if num == 1 or num == 2 then
		...
		local block = (targetID and (GG.DontFireRadar_CheckBlock(unitID, targetID) or GG.OverkillPrevention_CheckBlock(unitID, targetID, 400.1, 18))) or false
		return block
	else
		return false
	end
...
function script.Shot(num)
	if num == 1 or num == 2 then
		EmitSfx(gunPieces[num].flare, 1024)
		Move(gunPieces[num].recoil, z_axis, -10)
		Move(gunPieces[num].recoil, z_axis, 0, 10)
	elseif num == 3 then
